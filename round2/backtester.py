...
def run_backtest(trader, prices, trades, day, verbose=False):
    ...
    all_products = day_prices["product"].unique()
    last_order_depths: Dict[str, OrderDepth] = {}
    ...
    for tick_idx, ts in enumerate(timestamps):
        ...
        current_tick_products = tick_rows["product"].unique()

        # Update last known state for products seen in this tick
        for _, row in tick_rows.iterrows():
            product = row["product"]
            if pd.notna(row["mid_price"]) and row["mid_price"] > 0:
                last_order_depths[product] = build_order_depth(row)
                last_mid_prices[product] = row["mid_price"]
                last_rows[product] = row

        # Use ffilled data for all products
        order_depths: Dict[str, OrderDepth] = {p: last_order_depths[p] for p in all_products if p in last_order_depths}
        mid_prices: Dict[str, float] = {p: last_mid_prices[p] for p in all_products if p in last_mid_prices}
        ...
        if not day_trades.empty:
            ...
            for _, tr in tick_trades.iterrows():
                ...
                def clean_val(v): return str(v) if pd.notna(v) else ""
                
                all_trades.append({
                    "timestamp": int(ts),
                    "buyer": clean_val(tr.get("buyer")),
                    "seller": clean_val(tr.get("seller")),
                    "symbol": sym,
                    "currency": "SEASHELLS",
                    "price": int(tr["price"]),
                    "quantity": int(tr["quantity"])
        ...
        for product, orders in trader_result.items():
            ...
            if fills:
                ...
                for fill in fills:
                    ...
                    # Store for JSON log output
                    buyer = "SUBMISSION" if fill.buyer == "SUBMISSION" else "MARKET"
                    seller = "SUBMISSION" if fill.seller == "SUBMISSION" else "MARKET"
                    all_trades.append({
                        "timestamp": int(ts),
                        "buyer": buyer,
                        "seller": seller,
        ...
        for product in all_products:
            if product not in last_mid_prices:
                continue
            ...
            bp1, bv1 = row.get("bid_price_1", ""), row.get("bid_volume_1", "")
            bp2, bv2 = row.get("bid_price_2", ""), row.get("bid_volume_2", "")
            bp3, bv3 = row.get("bid_price_3", ""), row.get("bid_volume_3", "")
            ap1, av1 = row.get("ask_price_1", ""), row.get("ask_volume_1", "")
            ap2, av2 = row.get("ask_price_2", ""), row.get("ask_volume_2", "")
            ap3, av3 = row.get("ask_price_3", ""), row.get("ask_volume_3", "")

            def fmt(v): return str(int(v)) if pd.notna(v) else ""

            log_entry = (
                f"{day};{ts};{product};"
                f"{fmt(bp1)};{fmt(bv1)};{fmt(bp2)};{fmt(bv2)};{fmt(bp3)};{fmt(bv3)};"
                f"{fmt(ap1)};{fmt(av1)};{fmt(ap2)};{fmt(av2)};{fmt(ap3)};{fmt(av3)};"
                f"{mid_prices[product]};{pnl.get(product, 0.0)}"
            )
    ...
    return pd.DataFrame(results), activity_log, all_trades
...
def save_log(activity_log, trades_log, trader_name, day, output_dir="."):
    """Save activity log as official IMC JSON format."""
    fname = os.path.join(output_dir, f"backtest_{trader_name}_day{day}.log")
    payload = {
        "submissionId": "local-backtest",
        "activitiesLog": "\n".join(activity_log),
        "tradesLog": json.dumps(trades_log)
    }
...
def main():
    ...
    parser.add_argument("--log", "-l", action="store_true", help="Generate official log file")
    ...
    for day in days:
        ...
        results, activity_log, all_trades = run_backtest(trader, prices, trades, day, verbose=args.verbose)
        ...
        if args.log:
            save_log(activity_log, all_trades, trader_name, day)
